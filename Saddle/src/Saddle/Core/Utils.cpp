...
std::string Utils::ReadFile(const std::string& file_path)
{
    ...
    SADDLE_CORE_ASSERT_ARGS(in, "ReadFile: Could not open file: %s", file_path.c_str())
    ...
    SADDLE_CORE_ASSERT_ARGS(size != -1, "ReadFile: Could not read from file: %s", file_path.c_str());
